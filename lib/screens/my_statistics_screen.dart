...
class _MyStatisticsScreenState extends State<MyStatisticsScreen> {
  ...
  Future<void> _calculateStatistics() async {
    ...
    _avgConsumption =
        consumptionCount > 0 ? totalConsumption / consumptionCount : 0;

    // Total Monthly Spending
    final now = DateTime.now();
    _monthlySpending = fuelEntries
        .where((entry) =>
            DateTime.parse(entry['date']).month == now.month &&
            DateTime.parse(entry['date']).year == now.year)
        .fold(0.0, (sum, entry) => sum + entry['totalCost']);

    // Average Price Per Liter
    _avgPrice = fuelEntries.fold(
            0.0, (sum, entry) => sum + entry['pricePerLiter']) /
        fuelEntries.length;

    // Distance Driven in Last 30 Days
    final thirtyDaysAgo = now.subtract(const Duration(days: 30));
    final recentEntries = fuelEntries
        .where((entry) => DateTime.parse(entry['date']).isAfter(thirtyDaysAgo))
        .toList();
    if (recentEntries.length > 1) {
      final maxOdometer =
          recentEntries.map((e) => e['odometer']).reduce((a, b) => a > b ? a : b);
      final minOdometer =
          recentEntries.map((e) => e['odometer']).reduce((a, b) => a < b ? a : b);
